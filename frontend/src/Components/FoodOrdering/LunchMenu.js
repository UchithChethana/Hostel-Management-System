...
const LunchMenu = () => {
  ...
  useEffect(() => {
    const fetchFoods = async () => {
      try {
        const response = await fetch('http://localhost:7050/api/food/foodCategory/Lunch');
  ...
  const handleSelect = () => {
    // Navigate to selected foods page with selectedFoods state
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url('https://th.bing.com/th/id/R.c09cc81e4e1dbd86b9b01001ae8e719f?rik=iPTv8bpRV7ZD1Q&pid=ImgRaw&r=0')",
      }}
    >
      <div className="max-w-5xl mx-auto mt-20 mb-20 bg-slate-950 bg-opacity-90 rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-4xl text-white ml-96 font-bold">LUNCH</h1>
        </div>
        ...
        <div className="flex justify-center mt-6">
          <button
            ...
            className="bg-green-500 ml-96 w-44 text-black py-2 px-6 rounded-lg font-bold"
          >
            Select
          </button>
        </div>
